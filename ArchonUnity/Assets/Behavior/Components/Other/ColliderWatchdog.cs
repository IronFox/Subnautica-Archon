        ...
        {
            {
                ...
                if (currentlyInDisabled)
                {
                    if (c.enabled)
                    {
                        using (var log = Log.New())
                            log.Warn($"{nameof(ColliderWatchdog)}: Disabling collider {c.ComponentToString(transform)}");
                        c.enabled = false;
                    }
                }
                else
                {
                    ...
                    {
                        using (var log = Log.New())
                            log.Warn($"{nameof(ColliderWatchdog)}: Enabling collider {c.ComponentToString(transform)}");
                        c.enabled = true;
                    }
                    if (!c.gameObject.activeInHierarchy)
                    {
                        using (var log = Log.New())
                            log.Warn($"{nameof(ColliderWatchdog)}: Collider game object {c.ComponentToString(transform)} has been disabled. Fixing");
